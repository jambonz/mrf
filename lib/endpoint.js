...
const EVENT_NAME_ALIASES = {
  ...
  'dialogflow.ces.playback_started': 'dialogflow_ces::playback_started',
  'dialogflow.ces.playback_stopped': 'dialogflow_ces::playback_stopped',
...
class Endpoint extends EventEmitter {
  ...
  async api(command, args) {
     ...
     * xai, qwen, gptlive and voicelive all use the default mapping below
     * (host, path, authType, apiKey). */
    const s2 = /^uuid_(openai|voice_agent|ultravox|elevenlabs|assemblyai|google|xai|qwen|gptlive|voicelive)_s2s$/
      .exec(command);
    if (s2) {
      const vendor = s2[1];
      ...
      try {
        switch (cmd) {
          case 'session.create':
            ...
            } else if (vendor === 'voicelive') {
              /* azure voice live (own branch): args host, path, authType, apiKey
               * (host=<resource>.services.ai.azure.com,
               * path=voice-live/realtime?api-version=...&model=...,
               * authType=query for api-key or bearer for an Entra token) */
              await this._request('s2s.start', {
                vendor, host: parts[2], path: parts[3], authType: parts[4], apiKey: parts[5]
              });
            } else {
    ...
    if (df) {
      ...
      try {
        if (action === 'tool_result') {
          ...
          if (variant === 'ces') {
            /* '<uuid> <base64 json>' -> mid-stream send; CES's bidi stream owns
             * the turn awaiting the result, so a restart would abandon it.
             * base64 keeps a bare "'" in app-supplied text from splitting the arg. */
            const json = Buffer.from(t[0] || '', 'base64').toString('utf8');
            await this._request('dialogflow.toolResult', { toolResponses: JSON.parse(json) });
          } else if (variant === 'cx') {
            /* '<uuid> <project> <location> <agent> <env|none> <lang> '<json>''
             * -> primes a fresh turn; CX's stream is per-turn. */
            ...
            // ES has no client-side tool calls; don't guess at CX's arg layout
            return { body: `-ERR ${command}: tool results are cx/ces only` };
          }
  ...
  async join(conf, opts = {}) {
    ...
    // flags['dist-dtmf']: this member's DTMF is regenerated as RFC 2833 on
    // every other member's leg (mod_conference dist-dtmf semantics — the flag
    // is on the sender, recipients need nothing).
    if (flags['dist-dtmf']) data.distDtmf = true;
