...
const { parseSdp, translatePlayUrl, tokenizeQuoted } = require('./utils');
...
const EVENT_NAME_ALIASES = {
  ...
  // dialogflow (ES/CX/CES): mediajam emits dialogflow.<variant>.<suffix>;
  // the feature-server task (lib/tasks/dialogflow/index.js) registers raw
  // FreeSWITCH custom-event names per variant via addCustomEventListener.
  'dialogflow.es.intent': 'dialogflow::intent',
  'dialogflow.es.transcription': 'dialogflow::transcription',
  'dialogflow.es.end_of_utterance': 'dialogflow::end_of_utterance',
  'dialogflow.es.audio_provided': 'dialogflow::audio_provided',
  'dialogflow.es.error': 'dialogflow::error',
  'dialogflow.cx.intent': 'dialogflow_cx::intent',
  'dialogflow.cx.transcription': 'dialogflow_cx::transcription',
  'dialogflow.cx.end_of_utterance': 'dialogflow_cx::end_of_utterance',
  'dialogflow.cx.audio_provided': 'dialogflow_cx::audio_provided',
  'dialogflow.cx.tool_calls': 'dialogflow_cx::tool_calls',
  'dialogflow.cx.error': 'dialogflow_cx::error',
  'dialogflow.ces.session_output': 'dialogflow_ces::session_output',
  'dialogflow.ces.recognition_result': 'dialogflow_ces::recognition_result',
  'dialogflow.ces.interruption_signal': 'dialogflow_ces::interruption_signal',
  'dialogflow.ces.end_session': 'dialogflow_ces::end_session',
  'dialogflow.ces.go_away': 'dialogflow_ces::go_away',
  'dialogflow.ces.tool_calls': 'dialogflow_ces::tool_calls',
  'dialogflow.ces.audio_provided': 'dialogflow_ces::audio_provided',
  'dialogflow.ces.error': 'dialogflow_ces::error',
};
...
// A room-born conf-bot's fork.* events (bugname conf_bot:<room>:<member>) are
// reshaped into these conference::maintenance Action strings the conference
// verb routes to its nested listen task (conference-listen.js _onCustomEvent).
const CONF_BOT_ACTIONS = {
  'fork.connect': 'conf-bot-connect',
  'fork.connect_failed': 'conf-bot-connect-failed',
  'fork.disconnect': 'conf-bot-disconnect',
  'fork.error': 'conf-bot-error',
  'fork.transcription': 'conf-bot-transcription',
  'fork.json': 'conf-bot-json',
};
...
class Endpoint extends EventEmitter {
  ...
  async api(command, args) {
    ...
    if (command === 'uuid_jambonz_licensing') {
      if (arr[0] === 'generate-session-token') {
        /* nolicense mediajam build (hello licensing:false): no token to mint —
         * skip the control round-trip entirely. '+OK' with no token leaves the
         * feature-server's parser adding no header, exactly as the unlicensed
         * path did before. */
        if (this.ms.licensingEnabled === false) {
          this._sessionToken1 = '';
          return { body: '+OK' };
        }
        const callId = arr[2];
     ...
     * first then optional host/path (key rides a ?key= query param). openai,
     * xai, qwen and gptlive all use the default mapping below
     * (host, path, authType, apiKey). */
    const s2 = /^uuid_(openai|voice_agent|ultravox|elevenlabs|assemblyai|google|xai|qwen|gptlive)_s2s$/.exec(command);
    if (s2) {
      const vendor = s2[1];
        ...
        switch (cmd) {
          case 'session.create':
            if (vendor === 'elevenlabs') {
              ...
              /* xai voice agent (own branch): args host, path, authType, apiKey
               * (host=api.x.ai, path=v1/realtime?model=..., authType=bearer) */
              await this._request('s2s.start', {
    ...
    /* dialogflow (ES/CX/CES): the feature-server dialogflow task builds raw
     * FreeSWITCH api arg strings ('<uuid> <project> <lang> [event] [text]',
     * plus the CX/CES location/agent/environment-or-deployment/singleUtterance
     * variants) via _buildStartArgs/_startBotES; translate positionally into
     * mediajam dialogflow.* control commands. Credentials arrive via
     * ep.set('GOOGLE_APPLICATION_CREDENTIALS', ...) beforehand and are stashed
     * in _channelVars; forward them (and any other channel vars) as
     * data.options, matching the tts/stt vendor-options convention above. */
    const df = /^dialogflow(_cx|_ces)?_(start|stop|writesdone|tool_result)$/.exec(command);
    if (df) {
      const variant = df[1] === '_cx' ? 'cx' : (df[1] === '_ces' ? 'ces' : 'es');
      const action = df[2];
      try {
        if (action === 'tool_result' && variant === 'ces') {
          /* CES: '<uuid> <base64(JSON array of tool responses)>'
           * -> dialogflow.toolResult, delivered MID-STREAM on the live
           * session. Deliberately NOT dialogflow.start: CES's BidiRunSession
           * is one long-lived stream owning the agent's turn state, so a
           * restart would abandon the very turn that is awaiting the result.
           *
           * The payload is base64 so it survives this FreeSWITCH-style arg
           * string intact: tool output/error text is arbitrary app-supplied
           * JSON, and a bare "'" in it (e.g. "Can't reach CRM") would
           * otherwise terminate the quoted span early. */
          const raw = Array.isArray(args) ? args.join(' ') : String(args || '');
          const t = tokenizeQuoted(raw).slice(1); // drop leading <uuid>
          const json = Buffer.from(t[0] || '', 'base64').toString('utf8');
          await this._request('dialogflow.toolResult', { toolResponses: JSON.parse(json) });
        } else if (action === 'tool_result' && variant !== 'cx') {
          /* Only CX and CES have client-side tool calls. ES has no such concept
           * and mediajam's ES dialect ignores SessionConfig.ToolResult, so
           * falling through to the CX branch would parse the args with the wrong
           * positional layout and then silently RESTART the ES session with junk
           * fields, dropping the result. Reject it instead of guessing. */
          return { body: `-ERR ${command}: tool results are only supported for cx and ces` };
        } else if (action === 'tool_result') {
          // CX: '<uuid> <project> <location> <agent> <env|none> <lang> '<json>''
          // -> dialogflow.start primed with toolResult (new turn, same session);
          // correct for CX, whose StreamingDetectIntent stream is per-turn.
          const raw = Array.isArray(args) ? args.join(' ') : String(args || '');
          const t = tokenizeQuoted(raw).slice(1); // drop leading <uuid>
          const data = { variant, project: t[0], location: t[1], agent: t[2], language: t[4] };
          if (t[3] && t[3] !== 'none') data.environment = t[3];
          data.toolResult = JSON.parse(t[5]);
          data.options = { ...this._channelVars };
          await this._request('dialogflow.start', data);
        } else if (action === 'start') {
          const raw = Array.isArray(args) ? args.join(' ') : String(args || '');
          const t = tokenizeQuoted(raw).slice(1); // drop leading <uuid>
          let data;
          if (variant === 'es') {
            data = { variant, project: t[0], language: t[1] };
            if (t[2] !== undefined && t[2] !== 'none') data.event = t[2];
            if (t[3] !== undefined && t[3] !== 'none') data.text = t[3];
          } else {
            data = { variant, project: t[0], location: t[1], agent: t[2], language: t[4] };
            if (t[3] && t[3] !== 'none') data[variant === 'cx' ? 'environment' : 'deployment'] = t[3];
            if (t[5] && t[5] !== 'none') data.event = t[5];
            if (t[6] && t[6] !== 'none') data.text = t[6];
            data.singleUtterance = t[7] !== '0';
          }
          data.options = { ...this._channelVars };
          await this._request('dialogflow.start', data);
        } else if (action === 'stop') {
          await this._request('dialogflow.stop', {});
        } else {
          await this._request('dialogflow.writesDone', {});
        }
        return { body: '+OK' };
      } catch (err) {
        return { body: `-ERR ${err.message}` };
  ...
  async modify(sdp) {
    ...
    // drachtio-fsmrf's Endpoint.modify() resolves to the (re)negotiated local
    // SDP string, and the feature-server puts that value straight into the 200 OK
    // it sends back on a re-INVITE. Return the same here — returning `this` would
    // make the feature-server answer with a serialized Endpoint object as the SDP
    // body, corrupting media renegotiation (e.g. a carrier moving its RTP port).
    return this.local.sdp;
  }
  ...
  /* Move an existing (managed) listen fork into a conference room as a bot
   * member WITHOUT reconnecting its WebSocket — the bot's audio mixes into the
   * room (heard by all) and it hears the conference. Ownership transfers to the
   * room (it survives this caller's hangup and is reaped with the room). The
   * fork stays controllable by bugname (pause/resume/sendText/stop) and its
   * events keep arriving as fork.* on this endpoint. Returns {botMemberId,botId}. */
  async forkAdoptIntoConference(room, bugname) {
    const data = {room};
    if (bugname) data.bugname = bugname;
    const res = await this._request('room.fork.adopt', data);
    return {botMemberId: res?.botMemberId, botId: res?.botId};
  }

  /* Move an adopted fork back onto this endpoint (1:1), WS preserved; the bot
   * member is removed from the room. */
  async forkReleaseFromConference(bugname) {
    await this._request('room.fork.release', bugname ? {bugname} : {});
    return this;
  }

  /* Move this endpoint's live speech-to-speech session into a conference room
   * as a bot member WITHOUT reconnecting the vendor WebSocket — the agent hears
   * the conference and its audio mixes into the room (heard by all). Ownership
   * transfers to the room (it survives this caller's hangup and is reaped with
   * the room). The s2s.* events keep arriving on this endpoint's conn unchanged.
   * Returns {botMemberId, botId}. */
  async s2sAdoptIntoConference(room) {
    const res = await this._request('s2s.joinRoom', {room});
    return {botMemberId: res?.botMemberId, botId: res?.botId};
  }

  /* Move the adopted s2s session back onto this endpoint (1:1), WS preserved;
   * the bot member is removed from the room. */
  async s2sReleaseFromConference() {
    await this._request('s2s.leaveRoom', {});
    return this;
  }

  /* Nested-listen conf-bot (a fork born in a room): issued via this (the
   * conference's) endpoint so its conf-bot-* events route to the conference
   * verb on this endpoint's conn. Returns {botMemberId, botId}. */
  async startConferenceFork(room, opts = {}) {
    ...
    const password = opts.wsAuth?.password ?? this._channelVars.MOD_AUDIO_BASIC_AUTH_PASSWORD;
    if (username) data.wsAuth = {username, password: password || ''};
    const res = await this._request('room.fork.start', data);
    return {botMemberId: res?.botMemberId, botId: res?.botId};
  }

  async stopConferenceFork(room, member) {
    await this._request('room.fork.stop', {room, member});
    return this;
  }

  async sendConferenceForkText(room, member, text) {
    await this._request('room.fork.sendText', {room, member, text});
    return this;
  }

  /* Speak one-shot TTS into the room this endpoint is in, heard by every member.
   * sayUrl is the mediajam say: URL the feature-server builds from the
   * synthesizer + resolved credentials (exactly as the 1:1 say verb does), so
   * credential lookup stays feature-server-side. Synthesized at the room rate.
   * Emits say-start (first audio) / say-done (completed|stopped|replaced|
   * room-destroyed) on this conn, reshaped into conference::maintenance for the
   * conference verb. opts.id is an optional app correlation id echoed in events.
   * Returns {sayId, id?}. */
  async roomSay(sayUrl, opts = {}) {
    const room = opts.room || this._roomName;
    if (!room) throw new Error('roomSay: endpoint is not in a room');
    const data = {room, url: sayUrl};
    if (opts.id) data.id = opts.id;
    if (opts.playId) data.playId = opts.playId;
    if (opts.replace) data.replace = true;
    const res = await this._request('room.say', data);
    return {sayId: res?.sayId, ...(res?.id && {id: res.id})};
  }

  async stopRoomSay(opts = {}) {
    const room = opts.room || this._roomName;
    if (!room) return this;
    await this._request('room.sayStop', {room, ...(opts.sayId && {sayId: opts.sayId})});
    return this;
  }

  /* Play a file/tone into the room (the event-bearing sibling of the conference
   * entry beep): emits play-start (on accept) / play-done on this conn. Returns
   * {playId, id?}. */
  async roomPlay(url, opts = {}) {
    const room = opts.room || this._roomName;
    if (!room) throw new Error('roomPlay: endpoint is not in a room');
    const data = {room, url: translatePlayUrl(url)};
    if (opts.id) data.id = opts.id;
    if (opts.playId) data.playId = opts.playId;
    if (opts.replace) data.replace = true;
    const res = await this._request('room.play', data);
    return {playId: res?.playId, ...(res?.id && {id: res.id})};
  }

  async stopRoomPlay(opts = {}) {
    const room = opts.room || this._roomName;
    if (!room) return this;
    await this._request('room.playStop', {room, ...(opts.playId && {playId: opts.playId})});
    return this;
  }
  ...
  async _conferenceApi(command, args) {
    ...
    try {
      switch (verb) {
        ...
        case 'play': {
          // conference <conf> play <file> [<member>] — whole-room announcement /
          // entry beep. Map the FreeSWITCH bong tone_stream to a mediajam beep;
          // pass file/http urls through. (member-targeted play: Phase 2 TODO.)
          let url = rest[0];
          if (!url) return ok;
          if (url.startsWith('tone_stream://')) url = 'tone://?freq=440&duration=250&gainDb=-4';
          await this._request('room.play', {room, url});
          return ok;
        }
        case 'record':
          return ok; // conference recording — Phase 2 TODO
        default:
  ...
  _onEvent(evt, data) {
    // A room-born conf-bot's fork.* events carry bugname=conf_bot:<room>:<member>;
    // reshape into conference::maintenance conf-bot-* on this conn so the
    // conference verb (esl::event::CUSTOM::*) routes them to its listen child.
    if (evt.startsWith('fork.') && typeof data?.bugname === 'string' && data.bugname.startsWith('conf_bot:')) {
      const action = CONF_BOT_ACTIONS[evt];
      if (action) {
        const body = data.json ?? data.body ?? '';
        const headers = {
          'Event-Name': 'CUSTOM',
          'Event-Subclass': 'conference::maintenance',
          'Action': action,
          'Bot-ID': data.bugname,
          'Event-Date-Timestamp': String(Date.now() * 1000)
        };
        this._conn.emit('esl::event::CUSTOM::*', {
          getHeader: (n) => (n in headers ? headers[n] : null),
    ...
    switch (evt) {
      ...
      case 'play-start':
      case 'play-done': {
        // room say/play lifecycle → conference::maintenance so the conference
        // verb routes it (camelized: _onSayStart/_onSayDone/_onPlayStart/
        // _onPlayDone) and posts to its statusHook. Carries the optional app
        // correlation id; say-done/play-done carry reason + durationMs.
        const headers = {
          'Event-Name': 'CUSTOM',
          'Event-Subclass': 'conference::maintenance',
          'Action': evt,
          'Conference-Name': data.room || '',
          'Say-ID': data.sayId != null ? String(data.sayId) : '',
          'Play-ID': data.playId != null ? String(data.playId) : '',
          'App-ID': data.id != null ? String(data.id) : '',
          'Reason': data.reason || '',
          'Duration-Ms': data.durationMs != null ? String(data.durationMs) : '',
          'Vendor': data.vendor || '',
          'Tts-Error': data.error || '',
          'Event-Date-Timestamp': String(Date.now() * 1000)
        };
        this._conn.emit('esl::event::CUSTOM::*', {
          getHeader: (n) => (n in headers ? headers[n] : null)
