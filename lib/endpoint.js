...
class Endpoint extends EventEmitter {
  ...
  /**
   * Coach/whisper support (Phase 2): the set of conference members NOT carrying
   * the given tag, so the verb can `relate ... nospeak`. Until tags land,
   * return [] (no coaching applied).
   */
  async getNonMatchingConfParticipants() {
    return [];
  }
    ...
    try {
      switch (verb) {
        ...
        case 'set':
          ...
          return ok;
        // Phase 2 (room media, coach/whisper, conf-bot) — accept as no-ops so
        // the conference verb degrades gracefully until these land.
        case 'play':
        case 'record':
        case 'tag':
        case 'relate':
