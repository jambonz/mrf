      ...
      try {
        switch (cmd) {
          case 'session.create':
            ...
            } else if (vendor === 'gptlive') {
              /* openai GPT Live alpha (own branch — its own wire dialect, not
               * the realtime one): args host, path, authType, apiKey
               * (host=api.openai.com, path=v1/live?model=..., authType=bearer) */
              await this._request('s2s.start', {
                vendor, host: parts[2], path: parts[3], authType: parts[4], apiKey: parts[5]
              });
            } else {
