...
class MediaServer extends EventEmitter {
  ...
  _onHello(helloData) {
    ...
    /* codec lists (absent on older servers => empty): codecs is the server's
     * default offer order, supportedCodecs every codec a per-call `codecs`
     * list may name. The feature-server uses them to mirror an A leg's codec
     * ahead of the defaults and to validate user-supplied lists. */
    this.codecs = Array.isArray(helloData.codecs) ? helloData.codecs : [];
    this.supportedCodecs = Array.isArray(helloData.supportedCodecs) ? helloData.supportedCodecs : [];
  ...
  /* Start member-scoped listen forks on a room: one WebSocket fork per normal
   * member (present and future — late joiners are forked automatically), each
   * carrying ONLY that member's inbound audio (what the participant says,
   * never what they hear). Each fork's initial metadata frame identifies its
   * member ({room, memberId, callSid, accountSid, tag, sampleRate}, merged
   * over opts.metadata), so consumers label streams authoritatively. Forks die
   * with their member; the policy dies with the room. One policy per room.
   * Returns {members, count} — the member ids forked at start. */
  async startConferenceMemberForks(room, opts = {}) {
    const data = { room, wsUrl: opts.wsUrl };
    if (opts.sampleRate) data.sampleRate = opts.sampleRate;
    if (opts.metadata) data.metadata = opts.metadata;
    if (opts.wsAuth?.username) {
    ...
    const res = await this._connection.request('room.fork.members.start', null, data);
    return { members: res?.members || [], count: res?.count || 0 };
  }

  /** Stop a room's member-scoped listen forks (the whole policy), by room name. */
  async stopConferenceMemberForks(room) {
    await this._connection.request('room.fork.members.stop', null, { room });
    return this;
  }
