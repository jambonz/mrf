  ...
  _onFrame(frame) {
    switch (frame.t) {
      ...
      case 'res': {
        ...
        else if (frame.err?.code === 'unknown_endpoint' || frame.err?.code === 'unknown_session') {
          // Benign teardown race: the endpoint/session is already gone because
          // the call ended, which for a stop/cleanup command is the desired end
          // state. Resolve (not reject) so a best-effort teardown command issued
          // after destroy doesn't surface as an unhandled rejection.
          //
          // Resolve BEFORE logging, and guard the debug call: the injected logger
          // may not implement .debug (some callers pass an info/error-only logger),
          // and `logger?.debug(...)` still throws "debug is not a function" when
          // logger exists but the method doesn't — which would crash this frame
          // handler (an uncaught exception, taking the whole process down).
          p.resolve(frame.data || {});
          if (typeof this.logger?.debug === 'function') {
            this.logger.debug({id: frame.id, err: frame.err},
              'mediajam: command for already-gone endpoint/session; treating as no-op');
          }
        }
        else {
